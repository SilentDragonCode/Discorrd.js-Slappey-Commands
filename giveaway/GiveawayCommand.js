...
module.exports = class PurgeCommand extends BaseCommand {
  ...
  async run(client, message) {
    ...
    async function giveaway() {
      ...
      var time2 = '';
      var time3 = '';
      if (!message.member.hasPermission('MANAGE_MESSAGES')) return message.channel.send('You don\'t have enough permission to execute this command.');
      if (message.content === `p!giveaway`) return message.channel.send(`You didn\'t state a duration or a prize for the giveaway.`)
      if (message.content !== `p!giveaway`) {
          ...
          if (stated_duration_hours3 !== '0') {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                ...
                if (!msg.reactions.cache.get('🎉').users.cache.size < 1) {
                  const winner_embed = new Discord.MessageEmbed()
                  message.channel.send(`Winner: ${winner}\ndm ${message.author} to claim your rewards!`);
                }
              }, 1000);
